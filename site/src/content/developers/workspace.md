...
aos-ce/
├── crates/
│   ├── unicity-aos/             # product CLI
│   ├── unicity-aos-bootstrap/   # product home and migration
│   └── unicity-aos-health/      # loopback health projection
├── capsules/
│   ├── capsule-react/
│   ├── capsule-session/
...
`~/.unicity-os/runtime`. The `aos` process sets `ASTRID_HOME` only for the child
runtime. It does not mutate the user's shell environment or claim a standalone
Astrid installation.
