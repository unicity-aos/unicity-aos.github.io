/**
 * brand — the skin toggle. Astrid's own colours by default (this is the
 * upstream project's site); one click wears Unicity's brand (dark field,
 * orange accent, Anton + Inter) instead.
 * Pure token swap: an attribute on <html> and a lazily loaded font bundle,
 * persisted in localStorage. An inline head script applies the attribute
 * before first paint so a returning visitor never sees the wrong colours
 * flash.
 */
...
async function loadUnicityFonts(): Promise<void> {
  ...
  fontsLoaded = true;
  // The fonts only ever download for whoever flips the switch — Inter is
  // shared by both skins and loads statically from the layout.
  await Promise.all([
